...
test.describe("Admin Posts Management", () => {
  ...
  test("can navigate to posts page", async ({ page }) => {
    await page.locator('aside a[href="/admin/posts"]').click();
  ...
  test("can navigate to create new post", async ({ page }) => {
    ...
    const newPostBtn = page.locator(
      "a[href*='new'], a[href*='create'], button:has-text('New'), button:has-text('Create'), button:has-text('Add')"
    );
  ...
  test("new post form has required fields", async ({ page }) => {
    ...
    await expect(
      page.locator("input[name='titleEn'], input[name='title'], label:has-text('Title')").first()
    ).toBeVisible();
