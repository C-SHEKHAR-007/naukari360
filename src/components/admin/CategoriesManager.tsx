...
export default function CategoriesManager({
  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCategories([...categories, { ...cat, _count: { posts: 0 } }]);
  ...
  function startEdit(cat: Category) {
    setEditing(cat.id);
    setForm({ name: cat.name, nameHi: cat.nameHi || "", slug: cat.slug });
    setAdding(false);
  }
            ...
            <tr key={cat.id} className="hover:bg-surface/50">
              {editing === cat.id ? (
                <>
                  ...
                  <td className="px-4 py-2 text-muted">{cat._count.posts}</td>
                ...
                <>
                  ...
                  <td className="hidden px-4 py-3 text-muted sm:table-cell">{cat.nameHi || "—"}</td>
                  <td className="hidden px-4 py-3 text-muted md:table-cell">{cat.slug}</td>
